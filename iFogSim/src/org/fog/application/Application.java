...
public class Application {
	...
	public void addAppModule(String moduleName, int ram, int mips, int size, long bw) {
		//long bw = 1000;
		String vmm = "Xen";
	...
	// We need to create the DAG to convert the AppEdge relation between the nodes in the program. As we are saying
	// in the app that AppEdge is connecting two vertices, the DAG programatically defines this such as is M-0(source),
	// is connected with  M-1 and M-2 (Destination) for a Datatype DT-1, we set Adj (M-0) = {M-1, M-2}. So that,
	// while mapping the Modules with devices, we can create a connection between devices as well (service discovery)
	// . Such as, if we are placing M-1 in device D-1, we need to find its source Module(M-0)' placed device, say D-0
	// and make connection between D-1 and D-0.
