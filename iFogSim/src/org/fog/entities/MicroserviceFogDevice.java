...
//
//        System.out.println("Received tuple" + tuple.getCloudletId() + " with tupleType = " + tuple.getTupleType() +
//                "\t| Source : " + CloudSim.getEntityName(ev.getSource()) + "|Dest : " + CloudSim.getEntityName(ev.getDestination()));
//
//        if (Objects.equals(CloudSim.getEntityName(ev.getDestination()), "Resource_Event_Analysis_Microservice_2")){
//            System.out.println("Resource_Event_Analysis_Microservice_2 is here");
//        }
...
//                String destinationDetails = controllerComponent.getDestinationDeviceId(tuple.getDestModuleName());
//                if (destinationDetails.equals("-1")){
//                    System.out.println("Service discovery not found");
//                    return;
//                }
//                String[] parts = destinationDetails.split("\\*");
//
//                // The msInstance name is the first part
//                String msInstance = parts[0];
//                tuple.setDestModuleName(msInstance);

                // The deviceId is the second part, parsed as an integer
                int destination = controllerComponent.getDestinationDeviceId(tuple.getDestModuleName());//Integer.parseInt(parts[1]);
                if (destination == -1) {
                    //     System.out.println("Service DiscoveryInfo missing. Tuple routing stopped for : " + tuple
                    //     .getDestModuleName());
//                    if (!SimulationParameters.numberOfTUpleDelegatedToCloud.contains(tuple.getActualTupleId())) {
//                        SimulationParameters.numberOfTUpleDelegatedToCloud.add(tuple.getActualTupleId());
//                    }

                  //  destination = controllerComponent.getCloudId();

                    //===================Following code is for static optimisation ====================


                    int cloudId = controllerComponent.getCloudId();

                    for (FogDevice fogDevice : getControllerComponent().getFogDeviceList()) {
                        MicroserviceFogDevice microserviceFogDevice = (MicroserviceFogDevice) fogDevice;

//                        String destinationDetails1 =
//                                microserviceFogDevice.controllerComponent.getDestinationDeviceId(tuple.getDestModuleName());
//                        if (destinationDetails1.equals("-1")){
//                            System.out.println("Service discovery not found");
//                            return;
//                        }
//                        String[] parts1 = destinationDetails1.split("\\*");
//
//                        // The msInstance name is the first part
//                        String msInstance1 = parts1[0];
//                        tuple.setDestModuleName(msInstance1);

                        // The deviceId is the second part, parsed as an integer
                        destination = microserviceFogDevice.controllerComponent.getDestinationDeviceId(tuple.getDestModuleName());//Integer.parseInt(parts1[1]);
//
//                        if (msInstance1.contains("Resource_Event_Analysis_Microservice"))
//                        {
...
//                        }
                    ...
                    //===================Upper code is for static optimisation ====================

                    // ControllerComponent controllerComponent1 =


//                    Milestone milestone = (Milestone) getControllerComponent().getApplicationPerId(tuple.getAppId());
//
//                    MicroserviceFogDevice destinationDevice =
//                            controllerComponent.getMicroserviceFogDevice(destination);
//
//                    if (destinationDevice.getDeviceType() == MicroserviceFogDevice.CLOUD && milestone.toPlaceInCFs.contains(tuple.getDestModuleName())) {
//                        if (!SimulationParameters.numberOfTUpleDelegatedToCloud.contains(tuple.getActualTupleId())){
//                            SimulationParameters.numberOfTUpleDelegatedToCloud.add(tuple.getActualTupleId());
//                        }
//                        destination
//                    }
                    ...
                    if (destination == -1) {
                        System.out.println("Service Discovery Info missing. Tuple routing stopped for : " + tuple
                                .getDestModuleName());
                        return;
                    }else {
                        tuple.traversedMicroservices.put(tuple.getDestModuleName(), destination);
                        getControllerComponent().addServiceDiscoveryInfo(tuple.getSrcModuleName(), destination);
                    }
                }

//                Milestone milestone = (Milestone) getControllerComponent().getApplicationPerId(tuple.getAppId());
//
//                MicroserviceFogDevice microserviceFogDevice = controllerComponent.getMicroserviceFogDevice(destination);
//
//                if (microserviceFogDevice.getDeviceType() == MicroserviceFogDevice.CLOUD && milestone.toPlaceInCFs.contains(tuple.getDestModuleName())) {
//                    if (!SimulationParameters.numberOfTUpleDelegatedToCloud.contains(tuple.getActualTupleId())){
//                        SimulationParameters.numberOfTUpleDelegatedToCloud.add(tuple.getActualTupleId());
//                    }
//                }
            ...
            SimulationParameters.deviceToModuleToRunningState.get(getId()).put(tuple.getSrcModuleName(), false);
        }
        ...
        if (tuple.getDestinationDeviceId() == getId()) {
            ...
            AppModule module = getModuleByName(tuple.getSrcModuleName());
            double networkDelay = 0;
            if (module == null) {
              //  System.out.println(tuple.getSrcModuleName());
                if (!getAssociatedSensorAndDelayIds().isEmpty()) {

                    for (Pair<Integer, Double> sensorIdDelay : getAssociatedSensorAndDelayIds()) {
                        if (sensorIdDelay.getFirst() == tuple.getSourceModuleId()) {
                            networkDelay = sensorIdDelay.getSecond();
                            if (!SimulationParameters.moduleToNetworkDelays.containsKey(tuple.getSrcModuleName())) {
                                Double delayToGetData = networkDelay;
                                Double delayToSendData = 0.0;
                                Pair<Double, Double> latencies = Pair.create(delayToGetData, delayToSendData);
                                SimulationParameters.moduleToNetworkDelays.put(tuple.getDestModuleName(), latencies);
                            } else {
                                Pair<Double, Double> latencies = SimulationParameters.moduleToNetworkDelays.get(tuple.getDestModuleName());
                                if (latencies.getFirst() < networkDelay) {
                                    latencies = Pair.create(networkDelay, latencies.getSecond());
                                    SimulationParameters.moduleToNetworkDelays.put(tuple.getDestModuleName(), latencies);
                                }
                            }
                        }
                    }
                } else if (!getAssociatedActuatorIds().isEmpty()) {
                    for (Pair<Integer, Double> actuatorAssociation : getAssociatedActuatorIds()) {
                        ...
                        if (tuple.getSourceModuleId() == actuatorId) {
                            networkDelay = delay;
                        }
                    }
                }
                ...
                networkDelay = (double) tuple.getCloudletFileSize() / module.getBw();// getUplinkBandwidth();

                if (!SimulationParameters.moduleToNetworkDelays.containsKey(tuple.getDestModuleName())) {
                    Double delayToGetData = networkDelay;
                    Double delayToSendData = 0.0;
                    Pair<Double, Double> latencies = Pair.create(delayToGetData, delayToSendData);
                    SimulationParameters.moduleToNetworkDelays.put(tuple.getDestModuleName(), latencies);
            ...
            //     Microservice appModule = (Microservice) getModuleByName(tuple.getDestModuleName());
            //  List<ResCloudlet> status = appModule.getCloudletScheduler()


            if (SimulationParameters.deviceToModuleToRunningState.get(getId()).get(tuple.getDestModuleName()) == null){
                SimulationParameters.deviceToModuleToRunningState.get(getId()).put(tuple.getDestModuleName(), false);
            }else if (SimulationParameters.deviceToModuleToRunningState.get(getId()).get(tuple.getDestModuleName())){

                SimulationParameters.actualTupleInWaiting.put(tuple.getActualTupleId(), tuple);

                SimulationParameters.requestInWaiting++;
                UtilFactory.setServiceInWaiting(tuple.getDestModuleName(), getName(), tuple.getActualTupleId());
                //System.out.println("Module is busy");
            }
...
//                            }
    ...
    public void executeTuple(SimEvent ev, String moduleName) {
        Logger.debug(getName(), "Executing tuple on module " + moduleName);
        Tuple tuple = (Tuple) ev.getData();

        SimulationParameters.deviceToModuleToRunningState.get(getId()).put(moduleName, true);
        ...
        if (tuple.getDirection() == Tuple.UP) {
            ...
            // Following four lines add the source module and their ids for the specific destination module.
            if (!destModule.getDownInstanceIdsMaps().containsKey(srcModule))
                destModule.getDownInstanceIdsMaps().put(srcModule, new ArrayList<Integer>());
            if (!destModule.getDownInstanceIdsMaps().get(srcModule).contains(tuple.getSourceModuleId()))
                destModule.getDownInstanceIdsMaps().get(srcModule).add(tuple.getSourceModuleId());

            int instances = -1;
            for (String _moduleName : destModule.getDownInstanceIdsMaps().keySet()) {
                instances = Math.max(destModule.getDownInstanceIdsMaps().get(_moduleName).size(), instances);
            }
            destModule.setNumInstances(instances);
            //System.out.println("Module name: "+destModule.getName()+" number of instances: "+instances);
            System.out.println("Received module" + tuple.getCloudletId() + " with tupleType = " + tuple.getTupleType() +
                    "\t| Source : " + srcModule + "|Dest : " + destModule.getName());
        }
    ...
    public void initializeController(LoadBalancer loadBalancer, MicroservicePlacementLogic mPlacement, Map<Integer, Map<String, Double>> resourceAvailability, Map<String, Application> applications, List<FogDevice> fogDevices) {
        if (getDeviceType() == MicroserviceFogDevice.FON || getDeviceType() == MicroserviceFogDevice.CLOUD) {
            ...
            controllerComponent.updateResources(getId(), ControllerComponent.BW, getHost().getBw());
        } else
    ...
    public void initializeController(LoadBalancer loadBalancer, Map<String, Application> applications, List<FogDevice> fogDevices) {
        if (getDeviceType() != MicroserviceFogDevice.FON) {
            controllerComponent = new ControllerComponent(getId(), loadBalancer);
            controllerComponent.setApplicationInfo(applications);
            controllerComponent.setFogDeviceList(fogDevices);
            controllerComponent.updateResources(getId(), ControllerComponent.CPU, getHost().getTotalMips());
            controllerComponent.updateResources(getId(), ControllerComponent.RAM, getHost().getRam());
            controllerComponent.updateResources(getId(), ControllerComponent.STORAGE, getHost().getStorage());
            controllerComponent.updateResources(getId(), ControllerComponent.BW, getHost().getBw());
            ...
            for (Application app : perDevice.get(deviceID).keySet()) {
                if (MicroservicePlacementConfig.SIMULATION_MODE == "STATIC") {
                    ...
                    for (ModuleLaunchConfig moduleLaunchConfig : perDevice.get(deviceID).get(app)) {
                        ...
                        // Counting the deployed and created microservice instances
                        if (UtilFactory.microserviceTypeToInstanceNumber.containsKey(microserviceName)){
                            int currentInstance = UtilFactory.microserviceTypeToInstanceNumber.get(microserviceName);
                            UtilFactory.microserviceTypeToInstanceNumber.put(microserviceName, currentInstance + 1);
                        }else {
                            UtilFactory.microserviceTypeToInstanceNumber.put(microserviceName, 1);
                        }
                        ...
                        serviceDiscoveryAdd.put("service data", new Pair<>(microserviceName, deviceID));
                        serviceDiscoveryAdd.put("action", "ADD");
                        sendNow(deviceID, FogEvents.UPDATE_SERVICE_DISCOVERY, serviceDiscoveryAdd);

                        JSONObject serviceDiscoveryAdd1 = new JSONObject();
                        serviceDiscoveryAdd1.put("service data", new Pair<>(microserviceName, this.getId()));
                        serviceDiscoveryAdd1.put("action", "ADD");
                        sendNow(deviceID, FogEvents.UPDATE_SERVICE_DISCOVERY, serviceDiscoveryAdd1);
...
//        }
    ...
    protected void updateServiceDiscovery(SimEvent ev) {
        ...
        if (action.equals("ADD")) {
            if (placement.getFirst().equals("Resource_Event_Analysis_Microservice_2")) {
                System.out.println("Found");
            }
    ...
    protected void processModuleArrival(SimEvent ev) {
        ...
        Map<String, Boolean> moduleToRunning = new HashMap<>();
        moduleToRunning.put(module.getName(), false);
        SimulationParameters.deviceToModuleToRunningState.put(getId(), moduleToRunning);//.put(module.getName(), false);
        ...
        if (!appToModulesMap.get(appId).contains(module.getName())) {
            ...
            //SimulationParameters.deviceToModuleToRunningState.get(getId()).put(module.getName(), false);
        } else {
    ...
    protected void updateModuleInstanceCount(SimEvent ev) {
        ...
        if (getDeviceType() != FON) {
            ...
            double bw =
                    getControllerComponent().getAvailableResource(getId(), ControllerComponent.BW) - (config.getModule().getBw() * config.getInstanceCount());
            getControllerComponent().updateResources(getId(), ControllerComponent.BW, bw);
        }
