...
public class Tuple extends Cloudlet{
	...
	/*
	Keep track of execution time for this tuple in each microservice
	 */
	Map<String, Double> executionTimePerMicroservice = new HashMap<>();
	...
	public Map<String, Double> getExecutionTimePerMicroservice() {
		return executionTimePerMicroservice;
	}

	public void setExecutionTimePerMicroservice(Map<String, Double> executionTimePerMicroservice) {
		this.executionTimePerMicroservice = executionTimePerMicroservice;
	}

	private double appEdgeStartTime;

	public double getAppEdgeStartTime() {
		return appEdgeStartTime;
	}

	public void setAppEdgeStartTime(int sourceId, double currentTime) {
		// Set the input time for the tuple's application edge
		// In a more realistic scenario, you might need to handle multiple edges
		// and sources, but for simplicity, we'll assume a single source here.
		this.appEdgeStartTime = currentTime;

		// Log the event (you might want to adapt this based on your needs)
		System.out.println("Tuple " + tupleType + " received at Source " + sourceId +
				" at time " + currentTime + " for App Edge " + appEdgeStartTime);
	}
}
