...
public class FogDevice extends PowerDatacenter {
    ...
    protected double downLinkLatency;
    protected List<Pair<Integer, Double>> associatedActuatorIds;
    protected List<Pair<Integer, Double>> associatedSensorAndDelayIds;
    ...
    public FogDevice(
        ...
        setAssociatedSensorAndDelayIds(new ArrayList<Pair<Integer, Double>>());
    ...
    public AppModule getModuleByName(String moduleName) {
    ...
    /**
     * Sending periodic tuple for an application edge. Note that for multiple instances of a single source module,
     * only one tuple is sent DOWN while instanceCount number of tuples are sent UP.
     *
            ...
            for (Vm vm : host.getVmList()) {
                while (vm.getCloudletScheduler().isFinishedCloudlets()) {
                    ...
                    if (cl != null) {
                        ...
                        double executionTime = TimeKeeper.getInstance().tupleEndedExecution(tuple);
                        //calculateTheResponsivenessViolationFor(tuple, executionTime);
                        ...
                        for (Tuple resTuple : resultantTuples) {
                            ...
                            AppModule srcModule = getModuleByName(resTuple.getSrcModuleName());
                            double dataSendDelay = (double) resTuple.getCloudletFileSize() / srcModule.getBw();

                            if (!SimulationParameters.moduleToNetworkDelays.containsKey(resTuple.getDestModuleName())){
                                Double delayToGetData = 0.0;
                                Double delayToSendData = dataSendDelay;
                                Pair<Double, Double> latencies = Pair.create(delayToGetData, delayToSendData);
                                SimulationParameters.moduleToNetworkDelays.put(resTuple.getDestModuleName(), latencies);
                            }else {
                                Pair<Double, Double> latencies = SimulationParameters.moduleToNetworkDelays.get(resTuple.getDestModuleName());
                                if (latencies.getSecond() < dataSendDelay){
                                    latencies = Pair.create(latencies.getFirst(), dataSendDelay);
                                    SimulationParameters.moduleToNetworkDelays.put(resTuple.getDestModuleName(), latencies);
                                }
                            }
    ...
    private void calculateTheResponsivenessViolationFor(Tuple tuple, double executionTime) {

        Application application = getApplicationMap().get(tuple.getAppId());
        Microservice microservice = getMicroserviceByName(tuple, application);

//        if (microservice != null && getPlacedAppModulesPerApplication(application.getAppId()).contains(microservice.getName())) {
//            if (!SimulationParameters.appIdToTupleTypeToTime.containsKey(microservice.getName())){
//                double uplinkLatency = 0, downLinklatency = 0;
//                if (setUplinkLatency();)
//            }
//        }
    ...
    Microservice getMicroserviceByName(Tuple tuple, Application application) {

        Microservice microservice = null;
        for (AppModule appModule: application.getModules()){
            if (appModule.getName().equals(tuple.getDestModuleName())){
                microservice = (Microservice) appModule;
            }
        }
    ...
    public void updateAllocatedMips(String incomingOperator) {
    ...
    protected void sendTupleToActuator(Tuple tuple) {
        ...
        for (Pair<Integer, Double> actuatorAssociation : getAssociatedActuatorIds()) {
            ...
            if (tuple.getDestModuleName().equals(actuatorType)) {
               // SimulationParameters.goingToActuator++;
                send(actuatorId, delay, FogEvents.TUPLE_ARRIVAL, tuple);
                return;
            }
        }
    ...
    protected void processTupleArrival(SimEvent ev) {
        ...
        if (appToModulesMap.containsKey(tuple.getAppId())) { // Checking if this particular Fog device has the
            // appmodule of the app for which the tuple is arrived.
            if (appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())) { // Checking this
                // particular fog device has the tuple's destination module or not.
                int vmId = -1;
                for (Vm vm : getHost().getVmList()) {
                    // Finding the VM that is responsible for destination module of the tuple
                ...
                tuple.setVmId(vmId); // Setting the corresponding VM id which will process the tuple next.
    ...
    protected void processSensorJoining(SimEvent ev) {
        int sensorId = ev.getSource();
        double delay = (double) ev.getData();
        getAssociatedSensorAndDelayIds().add(new Pair<Integer, Double>(sensorId, delay));
        send(ev.getSource(), CloudSim.getMinTimeBetweenEvents(), FogEvents.TUPLE_ACK);
    }
    ...
    protected void sendUpFreeLink(Tuple tuple) {
        ...
        if (!SimulationParameters.moduleToNetworkDelays.containsKey(tuple.getDestModuleName())){
            Double delayToGetData = networkDelay + getUplinkLatency();
            ...
            if (latencies.getFirst() < networkDelay + getUplinkLatency()){
                latencies = Pair.create(networkDelay + getUplinkLatency(), latencies.getSecond());
                SimulationParameters.moduleToNetworkDelays.put(tuple.getDestModuleName(), latencies);
            }
        }
    ...
    protected void sendDownFreeLink(Tuple tuple, int childId) {
        ...
        if (!SimulationParameters.moduleToNetworkDelays.containsKey(tuple.getSrcModuleName())){
            Double delayToGetData = 0.0;//networkDelay + getUplinkLatency();
            Double delayToSendData = networkDelay + latency;
            Pair<Double, Double> latencies = Pair.create(delayToGetData, delayToSendData);
            SimulationParameters.moduleToNetworkDelays.put(tuple.getSrcModuleName(), latencies);
        }else {
            Pair<Double, Double> latencies = SimulationParameters.moduleToNetworkDelays.get(tuple.getSrcModuleName());
            if (latencies.getSecond() < networkDelay + latency){
                latencies = Pair.create(latencies.getFirst(), networkDelay + latency);
                SimulationParameters.moduleToNetworkDelays.put(tuple.getSrcModuleName(), latencies);
            }
        }
    ...
    public List<Pair<Integer, Double>> getAssociatedSensorAndDelayIds() {
        return associatedSensorAndDelayIds;
    }

    public void setAssociatedSensorAndDelayIds(List<Pair<Integer, Double>> associatedSensorAndDelayIds) {
        this.associatedSensorAndDelayIds = associatedSensorAndDelayIds;
    }
