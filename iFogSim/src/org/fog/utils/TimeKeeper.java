...
public class TimeKeeper {
    ...
    public static void setSimulationEndTime(double endTime) {
    ...
    public static boolean isSimulationRunning() {
        return CloudSim.clock() < simulationEndTime;
    }
    ...
    public double tupleEndedExecution(Tuple tuple) {
        ...
        tuple.getExecutionTimePerMicroservice().put(tuple.getDestModuleName(), executionTime);
        recordTheTimeToExecuteTheTupleByTheMicroservice(tuple, executionTime);
    ...
    private void recordTheTimeToExecuteTheTupleByTheMicroservice(Tuple tuple, double executionTime) {

        if (!SimulationParameters.appIdToTupleIdToTupleTypeToTime.keySet().contains(tuple.getAppId())) {
            Map<Integer, Map<String, Double>> tupleIdToTupleTypeToTime = new HashMap<>();
            Map<String, Double> tupleTypeToTime = new HashMap<>();
            tupleTypeToTime.put(tuple.getTupleType(), executionTime);
            tupleIdToTupleTypeToTime.put(tuple.getActualTupleId(), tupleTypeToTime);
            SimulationParameters.appIdToTupleIdToTupleTypeToTime.put(tuple.getAppId(), tupleIdToTupleTypeToTime);
        } else {
            Map<Integer, Map<String, Double>> tupleIdToTupleTypeToTime =
                    SimulationParameters.appIdToTupleIdToTupleTypeToTime.get(tuple.getAppId());
            if (!tupleIdToTupleTypeToTime.keySet().contains(tuple.getActualTupleId())) {
                ...
                Map<String, Double> tupleTypeToTime =
                        SimulationParameters.appIdToTupleIdToTupleTypeToTime.get(tuple.getAppId()).get(tuple.getActualTupleId());
                if (!tupleTypeToTime.keySet().contains(tuple.getTupleType())) {
                    tupleTypeToTime.put(tuple.getTupleType(), executionTime);
                    SimulationParameters.appIdToTupleTypeToTime.put(tuple.getAppId(), tupleTypeToTime);
                } else {
                    double updatedTime = tupleTypeToTime.get(tuple.getTupleType()) + executionTime;
                    tupleTypeToTime.put(tuple.getTupleType(), updatedTime);
                    SimulationParameters.appIdToTupleTypeToTime.put(tuple.getAppId(), tupleTypeToTime);
                }
