    ...
    private PlacementLogicOutput generatePlacementMap() {
        ...
        for (PlacementRequest placementRequest : placementRequests) {
            ...
            for (String microservice : mappedMicroservices.get(placementRequest.getPlacementRequestId()).keySet()) {
                if (placementRequest.getPlacedMicroservices().containsKey(microservice))
                    toRemove.add(microservice); // If this microservice is already in the cluster, no need to place them
                else
            ...
            for (String microservice : toRemove)
                mappedMicroservices.get(placementRequest.getPlacementRequestId()).remove(microservice); // Removing
            // the already placed microservices from the mapped module
        ...
        if (placement != null) {
            for (int prID : placement.keySet()) {
                ...
                // As the already placed microservices ar eremoved from the placement logic (actually form the
                // microservie mapped), need to check the corresponding placment requests (as it can contiant the
                // requests that are removed).
